...
# 7. O PULO DO GATO: Cria o arquivo .env físico em tempo de execução
# Esse comando vai rodar toda vez que o container ligar, injetando as variáveis da Render num arquivo real.
CMD ["/bin/sh", "-c", "\
    if [ -d '/var/www/html/livrosViajantes' ]; then TARGET='/var/www/html/livrosViajantes/.env'; else TARGET='/var/www/html/.env'; fi; \
